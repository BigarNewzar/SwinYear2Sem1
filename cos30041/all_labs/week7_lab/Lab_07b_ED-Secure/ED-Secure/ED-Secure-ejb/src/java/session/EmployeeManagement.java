...
@DeclareRoles({"ED-APP-ADMIN", "ED-APP-USER"})
@Stateless
public class EmployeeManagement implements EmployeeManagementRemote {
    ...
    public boolean checkUserIdPassword(String empId, String password) {
        Employee employee = employeeFacade.find(empId);
        if (employee == null) {
            System.out.println("its not finding data in table!");
            return false;
        }
        if (!(employee.getAppGroup().equals("ED-APP-USERS"))) {
            System.out.println("its not taking it as app user!");
            return false;
    ...
    public EmployeeDTO getEmpDetails(String empId, String password) {
        ...
        if (checkUserIdPassword(empId, password)) {
    ...
    private EmployeeDTO reviewEmpDetails(String empId) {
        Employee employee = employeeFacade.find(empId);
        if (employee != null) {
            EmployeeDTO empDTO = new EmployeeDTO(employee.getEmpid(), employee.getName(), employee.getPhone(), employee.getAddress(), employee.getEmail(), employee.getPassword(), employee.getAppGroup(), employee.getBnkAccId(), employee.getSalary(), employee.isActive());
            return empDTO;
        } else {
            return null;
        }
