...
//        return (salary > 0.0);
    ...
    public EmployeeDTO checkUserIdPassword() {
        boolean result = employeeManagement.checkUserIdPassword(empId, password);
    ...
    public String EchangeEmployee() {
        ...
        //check if this part is reallt needed or not
        EmployeeDTO e = employeeManagement.getEmployeeDetails(empId);
        EmployeeDTO p = employeeManagement.getEmployeeDetails(password);

        if (employeeManagement.checkUserIdPassword(empId, password) == true && result) {
            return "success";
        } else {
            return "failure";
        }
    ...
    public String EchangeEmployeePassword() {
        ...
        if (employeeManagement.checkUserIdPassword(empId, password) == true) {
    ...
    private String EsetEmployeeDetails() {
        ...
        System.out.println("testing before calling get emp details!");

        EmployeeDTO e = employeeManagement.getEmployeeDetails(empId);
        EmployeeDTO p = employeeManagement.getEmployeeDetails(password);
        
        System.out.println("testing if this can run!");

        if (e == null) {
            // no such employee
            return "failure";
        }
        if (employeeManagement.checkUserIdPassword(empId, password)) {
            
            System.out.println("testing if check password can run!");
            // found - set details for display
            this.empId = e.getEmpid();
            this.name = e.getName();
            this.phone = e.getPhone();
